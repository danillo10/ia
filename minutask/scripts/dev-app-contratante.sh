#!/usr/bin/env bash
# App contratante com hot reload (Chrome + reload ao salvar arquivos)
set -euo pipefail

# shellcheck source=flutter-dev-env.sh
source "$(dirname "$0")/flutter-dev-env.sh"

APP_DIR="$ROOT/app-contratante"
PORT="${MINUTASK_PORT_CONTRATANTE:-4200}"
PID_FILE="${TMPDIR:-/tmp}/minutask-contratante.pid"
    ...
    while inotifywait -r -q -e close_write,move,create,delete "$APP_DIR/lib" 2>/dev/null; do
      if [[ -f "$PID_FILE" ]]; then
        if kill -USR1 "$(cat "$PID_FILE")" 2>/dev/null; then
          echo "⟳ Hot reload contratante ($(date +%H:%M:%S))"
        fi
      fi
    done
  ) &
...
echo "Contratante → http://localhost:$PORT"
echo "API → $API_BASE"
